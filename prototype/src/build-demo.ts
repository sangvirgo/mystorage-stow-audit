...
<header>
  ...
  <p class="muted">Stow is MyStorage's customer chat assistant. In my audit some of its answers were wrong or incomplete. This page shows, for each problem, <b>what Stow really said</b>, <b>what a correct answer needs</b>, and <b>what my fixed prototype says</b> to the same question.</p>
  <ol class="muted">
    <li>Pick a customer question in the list.</li>
    <li>Left: Stow's real answer, marked ✗ if it fails. Middle: what a good answer must contain. Right: my prototype's answer, marked ✓ if it passes.</li>
    <li>The table gives the score over all questions. Every number comes from <code>results/*.json</code>.</li>
  </ol>
</header>
<main>
  <section aria-labelledby="totals"><h2 id="totals">Score over all questions (answers that pass every check / answers scored)</h2><div id="totals-table"></div></section>
  <div class="controls">
    <label for="case">Customer question</label><select id="case"></select>
    <span id="runs" role="group" aria-label="Prototype run (each question was answered 3 times)"></span>
  </div>
  <div class="grid" aria-live="polite">
    <section class="panel" aria-labelledby="orig-h"><h2 id="orig-h">1. What Stow said</h2><div id="orig"></div></section>
    <section class="panel" aria-labelledby="ev-h"><h2 id="ev-h">2. What a good answer needs</h2><div id="evidence"></div></section>
    <section class="panel" aria-labelledby="prop-h"><h2 id="prop-h">3. What my prototype says</h2><div id="prop"></div></section>
  </div>
</main>
<footer class="small muted">
  ...
  <ul>
    ...
    <li><b>Plain prompt</b> and <b>Prototype</b> use the same model and the same source registry. The prototype adds a rules prompt, language and percentages decided in code, and a runtime check with at most one repair call.</li>
...
const REPORT_NO = { A: 'F-5', B: 'F-6', C: 'F-1', D: 'F-7', E: 'F-7', F: 'F-2', G: 'F-3', H: 'F-8', M: 'minor note' };
const label = (t) => t.replace(/\\bF-([A-M])\\b/g, (m, l) => 'report ' + REPORT_NO[l]);
...
function renderTotals() {
  const t = el('table');
  t.append(el('tr', {}, ...['Set','Plain prompt (same model)','Prototype, first pass','Prototype, final','Stow (recorded)'].map((h) => el('th', { scope: 'col', textContent: h }))));
...
const badge = (pass) => el('span', { className: 'badge ' + (pass ? 'pass' : 'fail'), textContent: pass ? '✓ passes every check' : '✗ fails at least one check' });
...
function panel(label, row, ans) {
  const box = el('div');
  box.append(el('p', { className: 'small muted', textContent: label }), badge(ans.pass));
  const other = otherFailures(row, ans);
  if (other.length) box.append(el('p', { className: 'small', textContent: 'Failed check: ' + other.join('; ') }));
  box.append(answerBlock(ans));
  return box;
}
...
function render() {
  ...
  const origLabel = stowRecorded ? 'Stow, real production reply (' + row.stowTranscript + ')' : 'Plain prompt, same model (run 1). Stow was not asked this question in production, so the plain prompt stands in.';
  ...
  const ev = el('div');
  ev.append(el('h3', { textContent: 'Customer asked' }), el('div', { className: 'answer', textContent: row.question }), el('p', { className: 'badge note', textContent: 'Problem: ' + label(row.finding) }));
  const list = el('ul', { className: 'checks' });
  const mark = (ans, text) => !ans ? '-' : failedHas(ans, text) ? '✗ fails' : '✓ ok';
  for (const [kind, texts] of [['Must include', row.expectedFacts], ['Must not say', row.prohibited]]) {
    for (const t of texts) list.append(el('li', {}, el('b', { textContent: kind + ': ' }), t, el('br'), el('span', { className: 'small muted', textContent: 'Stow: ' + mark(origAns, t) + '   Prototype: ' + mark(prop, t) })));
  }
  ev.append(el('h3', { textContent: 'Checklist' }), list);
  if (prop && prop.violations.length) {
    ev.append(el('h3', { textContent: 'Safety check inside the prototype' }), el('p', { className: 'badge note', textContent: 'a problem was caught and repaired before answering' }), el('p', { className: 'small', textContent: prop.violations.join(' | ') + (prop.remaining.length ? ' Still broken: ' + prop.remaining.join(' | ') : ' Fixed after one repair call.') }));
  }
  const d = el('details');
  ...
  document.getElementById('prop').replaceChildren(prop ? panel('Prototype, run ' + (current.run + 1) + ' of ' + row.proposed.length + ' (' + D[current.set].model + ')', row, prop) : el('p', { textContent: 'Not run.' }));
...
function init() {
  ...
  for (const set of ['dev', 'holdout']) {
    const g = el('optgroup', { label: set === 'dev' ? 'A. Questions from my audit findings (used while building the fix)' : 'B. Extra questions written before tuning (to check the fix is not over-fitted)' });
    for (const r of rows[set]) g.append(el('option', { value: set + ':' + r.id, textContent: r.id + ' - ' + label(r.finding) }));
    sel.append(g);
  }
